...
def main():
    ...
    # Integration Name
    integration["Name"] = "INTXXX Testing Studio Build v1"
    ...
    while True:
        integration_template_name = "Cloud Integration Template"
    ...
    if integration["Template"] == "Enterprise Interface Builder":
        ...
        if create_report_tf == "Y":
            ...
            if data_source_filter_list[data_source]:
                ...
                while True:
                    ...
                    data_source_filter = "All Employees"
        ...
        # Create Request
        request = create_custom_report(integration, credentials)
        header = {"content-type": "text/xml"}
    ...
    request = create_integration_system(integration, credentials)
    ...
    # Create ISU
    request = create_ISU(integration, credentials)
    r = requests.post(wsdls["Integrations"], data=request, headers=header)
    ...
    # Create ISSG
    request = create_ISSG(integration, credentials)
    r = requests.post(
        wsdls["Core_Implementation_Service"], data=request, headers=header
    )
